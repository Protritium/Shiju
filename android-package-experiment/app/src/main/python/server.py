...
ROOT = Path(os.environ.get("SHIJU_ROOT", Path(__file__).resolve().parent)).resolve()
BOOKS = {
...
def normalize_book(raw: object) -> dict:
    ...
    if isinstance(sync.get("answerState"), dict):
        for word_id, item in sync["answerState"].items():
            if isinstance(item, dict) and isinstance(item.get("value"), str) and item["value"].strip() and isinstance(item.get("updatedAt"), str): result["sync"]["answerState"][str(word_id)] = {"value": item["value"].strip()[:120], "updatedAt": item["updatedAt"]}
    ...
    if isinstance(sync.get("reviewEvents"), dict): result["sync"]["reviewEvents"] = {str(word_id): sorted({str(day) for day in days if re.fullmatch(r"\d{4}-\d{2}-\d{2}", str(day))}) for word_id, days in sync["reviewEvents"].items() if isinstance(days, list)}
