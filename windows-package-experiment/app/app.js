...
let pendingDictionaryWord = "";
let dictionaryConfirmTrigger = null;
...
function stopSpeech() {
  ...
  if (nativeSpeechAudio) {
    nativeSpeechAudio.pause();
    nativeSpeechAudio.removeAttribute("src");
    nativeSpeechAudio.load();
    nativeSpeechAudio = null;
  }
  if ("speechSynthesis" in window) window.speechSynthesis.cancel();
  setSpeechState(null, "");
}

function windowsNativeSpeak(text, rate, button) {
  const source = `/api/tts?text=${encodeURIComponent(text)}&rate=${encodeURIComponent(rate)}&_=${Date.now()}`;
  const audio = new Audio(source);
  nativeSpeechAudio = audio;
  audio.preload = "auto";
  audio.onplaying = () => setSpeechState(button, "正在使用 Windows 本地英语语音");
  audio.onended = () => {
    if (nativeSpeechAudio === audio) nativeSpeechAudio = null;
    setSpeechState(null, "");
  };
  audio.onerror = () => {
    if (nativeSpeechAudio !== audio) return;
    nativeSpeechAudio = null;
    setSpeechState(null, "Windows 本地语音不可用，正在尝试浏览器语音…");
    browserSpeak(text, rate, button, 0);
  };
  audio.play().catch(() => audio.onerror());
}

function browserSpeak(text, rate, button, retry = 0) {
  ...
  if (!voices.length && retry < 8) {
    ...
    speechTimer = setTimeout(() => browserSpeak(text, rate, button, retry + 1), 350);
    return;
  }
  ...
  utterance.rate = rate;
  utterance.pitch = 1;
  utterance.onstart = () => setSpeechState(button, voice ? `正在使用浏览器英语语音 · ${voice.name}` : "正在使用浏览器默认英语语音");
...
function speakText(text, rate, button) {
  stopSpeech();
  if (!text) return;
  try {
    if (window.ShijuTTS?.status && window.ShijuTTS?.speak) {
      ...
      if (status === "ready" && window.ShijuTTS.speak(text, rate)) {
        setSpeechState(button, "正在使用系统离线英语语音");
        speechTimer = setTimeout(() => setSpeechState(null, ""), Math.max(1800, text.length * 105 / rate));
        return;
      }
      if (status === "missing") {
        setSpeechState(null, "未安装离线英语语音，正在打开系统语音安装页面…");
        if (window.ShijuTTS.installVoiceData) window.ShijuTTS.installVoiceData();
        return;
      }
      setSpeechState(null, "系统英语语音正在初始化，请稍后重试。");
      return;
    }
  } catch {}
  windowsNativeSpeak(text, rate, button);
}

function speakCurrentWord() {
  const item = state.queue[state.index];
  if (item) speakText(item.word, 0.76, $("#speakWordBtn"));
}
...
function speakCurrentContext() {
  ...
  if (!item) return;
  const content = state.format === "phrase" ? item.phrase?.phrase : item.example?.sentence;
  speakText(content, 0.9, $("#speakContextBtn"));
}
...
function openDictionaryPopup(word) {
  ...
  dictionaryConfirmTrigger = document.activeElement;
  $("#dictionaryConfirmWord").textContent = word;
  $("#dictionaryConfirm").classList.remove("hidden");
  $("#confirmDictionaryLookup").focus();
}

function cancelDictionaryLookup() {
  $("#dictionaryConfirm").classList.add("hidden");
  pendingDictionaryWord = "";
  if (dictionaryConfirmTrigger instanceof HTMLElement) dictionaryConfirmTrigger.focus();
  dictionaryConfirmTrigger = null;
}

function confirmDictionaryLookup() {
  const word = pendingDictionaryWord;
  $("#dictionaryConfirm").classList.add("hidden");
  pendingDictionaryWord = "";
  dictionaryConfirmTrigger = null;
  if (!word) return;
...
function closeGuide() {
  ...
  if (!state.revealed) setTimeout(() => $("#answerInput").focus(), 30);
}
...
async function exportLearningData() {
  ...
  try {
    ...
    const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([json], { type: "application/json" })); link.download = filename; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000);
    $("#dataSyncStatus").textContent = "备份已生成，请保存到需要同步的设备。";
...
$("#cancelDictionaryLookup").addEventListener("click", cancelDictionaryLookup);
$("#confirmDictionaryLookup").addEventListener("click", confirmDictionaryLookup);
$("#dictionaryConfirm").addEventListener("click", event => { if (event.target === $("#dictionaryConfirm")) cancelDictionaryLookup(); });
...
document.addEventListener("keydown", event => {
  ...
  if (!$("#dictionaryConfirm").classList.contains("hidden")) {
    if (event.key === "Escape") cancelDictionaryLookup();
    if (event.key === "Enter") confirmDictionaryLookup();
    event.preventDefault();
    return;
  }
  ...
  if (event.key === "F2" && !event.repeat) {
    if (!$("#authOverlay").classList.contains("hidden") || !$("#wordPopup").classList.contains("hidden")) return;
    event.preventDefault();
    speakCurrentContext();
    return;
  }
